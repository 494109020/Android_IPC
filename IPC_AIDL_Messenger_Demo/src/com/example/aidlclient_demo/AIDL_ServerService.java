...
public class AIDL_ServerService extends Service {
	...
	// 2.IPC调用是同步的。如果你知道一个IPC服务需要超过几毫秒的时间才能完成地话，你应该避免在Activity的主线程中调用。也就是IPC调用会挂起应用程序导致界面失去响应，这种情况应该考虑单独开启一个线程来处理。反之亦然
	// 3.抛出的异常是不能返回给调用者（跨进程抛异常处理是不可取的）
	// 4.aidl文件中定义接口的参数
	// 视情况需要有方向指示，包括in、out和inout，in表示由客户端设置，out表示由服务端设置，inout是两者均可设置。
	// 5.记得要做权限验证， 可以在onBinder中做，也可以在onTransact中做，方法不唯一
	private AIDL_IBinderPoolImpl impl;
